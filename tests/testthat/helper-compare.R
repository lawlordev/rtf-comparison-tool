...
# Locates the project root (the folder containing R/, tests/, examples/), loads
# the comparison engine, and defines path helpers: fx() for the small fixtures,
# ex() for the large example files.
...
.rtf_find_root <- function() {
  ...
  stop("helper-compare.R: could not locate R/compare_rtf.R (project root).")
}

RTF_ROOT <- .rtf_find_root()
if (!exists("compare_rtf", mode = "function")) {
  source(file.path(RTF_ROOT, "R", "compare_rtf.R"))
}
